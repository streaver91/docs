...
The code can be found in the OpenKIM repository with ID \lstinline{00000000000000}.
...
And we defined the expression in the middle as $\mathit{g(V_0)}$, thus $\mathit{VFE} = g(\infty)$.
...
For sc, fcc, bcc, and diamond structures, we used the conventional cubic cell as the basis for constructing supercells, and for hcp structures, we also used orthorhombic cells (with lattice vectors $[a, \sqrt{3}a, c]$) as the basis.
...
The downward trend can be qualitatively explained by the fact that, when the relaxation volume is large, more energy is needed for the vacancy to pull the neighboring atoms towards itself.
